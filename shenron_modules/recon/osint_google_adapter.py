...
# Optional registry import (no-op if missing during tests)
try:
    from shenron_core import payload_registry  # type: ignore[attr-defined]
except Exception:
    payload_registry = None  # type: ignore[assignment]

VENDOR_REL = Path("shenron_modules/recon/vendor/osint_google_raw.py")
...
def run_adapter(params: Dict[str, Any]) -> Dict[str, Any]:
    """
    Execute the vendor OSINT recon script in a subprocess.
    Tries once WITH flags, then retries ONCE WITHOUT flags if the first run fails.
    ...
    ts = _timestamp()
    run_dir = OUT_DIR / f"osint_google_{ts}"
    run_dir.mkdir(parents=True, exist_ok=True)
    log_file = LOG_DIR / f"osint_google_{ts}.log"
    ...
    zip_path = OUT_DIR / f"osint_google_{ts}.zip"
...
def _register() -> None:
    ...
    if payload_registry and hasattr(payload_registry, "register_payload"):
        ...
        @payload_registry.register_payload(
            name="recon.osint_google",
            version="0.1.0",
            description=(
                "OSINT Google recon adapter (executes vendor script in a sandboxed subprocess). "
                "Use only in authorized lab environments."
            ),
            tags=["recon", "web", "osint", "adapter", "research-only"],
        )
