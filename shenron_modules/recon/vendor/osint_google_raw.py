...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="OSINT Google vendor (non-interactive, URLs only).")
    p.add_argument("--query", default=os.getenv("SHENRON_QUERY", "site:example.com"))
    p.add_argument(
        "--max-pages", type=int, dest="max_pages", default=int(os.getenv("SHENRON_MAX_PAGES", "10"))
    )
...
def main() -> int:
    ...
    results = list(search(args.query, num_results=max(0, args.max_pages)))
