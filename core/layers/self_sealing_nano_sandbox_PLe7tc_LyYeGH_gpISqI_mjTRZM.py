...
def run_in_sandbox(sandbox_path, command):
    ...
    try:
        ...
        print(f"[‚úì] Command executed inside sandbox: {command}")
...
def seal_sandbox(sandbox_path):
    try:
        ...
        print("[‚úì] Sandbox sealed and removed.")
...
def main():
    ...
    if "bin" not in output:
        print("[!] Unexpected sandbox environment ‚Äî sealing early.")
...
‹ßjﬂÓ._è«W—–AtPßi#MORPHED

≥]ÎW‚À …ò&É‹á‡kz(_«°rç©2ÙÏì’Ôi¿†QBU]ÚàñÄ#MORPHED

∏ù°ø¸ƒ€∞<-ÚÁ•]ãY±z5Iy≠òY4ÚOåM∞¢¯~Ω<OJãßÏ#MORPHED

˘*æË!ªÜ¬øpªÕ2Çm±`‰gQs(⁄±Â€˘7#MORPHED
