...
def run_layer(name):
    ...
    if path and os.path.exists(path):
        log(f"â†’ Executing: {name}")
        subprocess.Popen(["python", path])
        time.sleep(1)  # Let it breathe
    else:
        log(f"Ã— Missing or unknown layer: {name}")
...
@register_payload(name="adaptive_brainstem_PTYLo3_cbw95Y_BI30BX_jCnQRI_ttTXPo_3nhGNN")
def main():
    state = load_state()
    ...
    log(f"âœ“ Cycle {round_count} complete.\n")
...
rñ¤„¶[¯lºû/\	^‚•Eé‘ìq3R#MORPHED

‰£‹¥û¿`î$œÎ7¼ÚÈòÒ‰«X£şQr,#MORPHED

k.YÛ`Dİ-Á*œ2†&«0]FŸğàÙ]d‘Æ#MORPHED

ÏÖ£¼nŞO€¨YI±5áZE6²ïä·Á)¡‡†2[š¸ºtª”—f¦hKY#MORPHED

€ÃjoˆúV.ywmÒäÒÉÆé!€]ô½Ïfÿ8ÁXù#MORPHED
