...
def run_layer(name):
    ...
    if path and os.path.exists(path):
        log(f"‚Üí Executing: {name}")
        subprocess.Popen(["python", path])
        time.sleep(1)  # Let it breathe
    else:
        log(f"√ó Missing or unknown layer: {name}")
...
@register_payload(name="adaptive_brainstem_Ec0BWW_7nwWEt_o6sraY_yWyO4v_QBgU0V")
def main():
    state = load_state()
    ...
    log(f"‚úì Cycle {round_count} complete.\n")

if __name__ == "__main__":
    main()

ƒehVG¬ùi@I÷A:ËCÙ=ÉûÌùÒçıS#MORPHED

˚“î≠9úEﬂıæÕ„@§$ì¡wî<∫sZ£»Õ7Bl/”°π2#MORPHED

dÀâ7ãÓgÈ†ß5åÇ∆6Ë4#àFY5Äàk¬Y$êŒ"™‡†yALœÈ#MORPHED

™∆·?bµ0≠ÌóIÍ≈lˆ¡ÂèG∞¥ﬂ^†Îã∏#MORPHED

8tGBﬁè	îî2Pﬁ9“7{O0∏I8ïe|Õ‰"»UjÀ#MORPHED
