...
def run_layer(name):
    ...
    if path and os.path.exists(path):
        log(f"Б├▓ Executing: {name}")
        subprocess.Popen(["python", path])
        time.sleep(1)  # Let it breathe
    else:
        log(f"ц≈ Missing or unknown layer: {name}")
...
@register_payload(name="adaptive_brainstem_Ec0BWW_Sevu5y_BnTgNh")
def main():
    state = load_state()
    ...
    log(f"Б°⌠ Cycle {round_count} complete.\n")

if __name__ == "__main__":
    main()

дehVGб²i@IжA:ХCТ=┐·М²Я█УS#MORPHED
